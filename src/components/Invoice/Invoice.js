...
const Invoice = () => {
    ...
    const [currency, setCurrency] = useState(currencies[38].value)
  ...
  const handleRates =(e) => {
    ...
    const handleAddField = (e) => {
        e.preventDefault()
        setInvoiceData((prevState) => ({...prevState, items: [...prevState.items,  {truckNumber: '', ticketNumber:'', hauledFrom:'', hauledTo:'' , unitPrice: '', quantity: '', discount: '', amount: '' }]}))
    }
    ...
    const handleSubmit =  async (e ) => {
        ...
        if(invoice) {
         ...
         history.push(`/invoice/${invoice._id}`)
            ...
            console.log("invoiceData before dispatch: ", invoiceData)
        dispatch(createInvoice({
            ...
            status: status, 
            paymentRecords: [], 
            creator: [user?.result?.sub || user?.result?.googleId] }, 
            history
            ))
        }
            ...
            <Container>
                <Grid container justifyContent="space-between" style={{marginTop: '40px'}} >
                    ...
                    <Grid item style={{marginRight: 20, textAlign: 'right'}}>
                        ...
                        <Typography variant="h6" gutterBottom>{currency} {toCommas(total.toFixed(2))}</Typography>
                    </Grid>
                </Grid>
            </Container>
    ...
    <div>
        ...
        <Table className={classes.table} aria-label="simple table">
            ...
            <TableRow>
                ...
                <TableCell>Hauled from</TableCell>
                <TableCell>Hauled to</TableCell>
                <TableCell >Net Weight(Tonnes)</TableCell>
                <TableCell>Price Per Tonne</TableCell>
            ...
            {invoiceData.items.map((itemField, index) => (
                <TableRow key={index}>
                <TableCell  scope="row" style={{width: '20%' }}> <InputBase style={{width: '100%'}} outline="none" sx={{ ml: 1, flex: 1 }} type="text" name="truckNumber" onChange={e => handleChange(index, e)} value={itemField.truckNumber} placeholder="Enter Truck number" /> </TableCell>
                <TableCell  scope="row" style={{width: '20%' }}> <InputBase style={{width: '100%'}} outline="none" sx={{ ml: 1, flex: 1 }} type="text" name="ticketNumber" onChange={e => handleChange(index, e)} value={itemField.ticketNumber} placeholder="Enter Ticket number" /> </TableCell>
                <TableCell  scope="row" style={{width: '30%' }}> <InputBase style={{width: '100%'}} outline="none" sx={{ ml: 1, flex: 1 }} type="text" name="hauledFrom" onChange={e => handleChange(index, e)} value={itemField.hauledFrom} placeholder="Haulage from" /> </TableCell>
                <TableCell  scope="row" style={{width: '30%' }}> <InputBase style={{width: '100%'}} outline="none" sx={{ ml: 1, flex: 1 }} type="text" name="hauledTo" onChange={e => handleChange(index, e)} value={itemField.hauledTo} placeholder="Haulage to" /> </TableCell>
        ...
        <div className={styles.invoiceSummary}>
            ...
            <div className={styles.summaryItem}>
                <p>Sub total:</p>
                <h4>{toCommas(subTotal.toFixed(2))}</h4>
            </div>
            <div className={styles.summaryItem}>
                <p>VAT(%):</p>
                <h4>{toCommas(vat.toFixed(2))}</h4>
            </div>
            <div className={styles.summaryItem}>
                <p>Total</p>
                <h4 style={{color: "black", fontSize: "18px", lineHeight: "8px"}}>{currency} {toCommas(total.toFixed(2))}</h4>
            </div>
