...
const Invoices = () => {
  ...
  // const rows = []
    ...
    useEffect(() => {
      dispatch(getInvoicesByUser({ search: user?.result?.sub || user?.result?.googleId}));
      // eslint-disable-next-line
    },[location])
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row._id} style={{cursor: 'pointer'}} >
                ...
                <TableCell style={tableStyle} onClick={() => openInvoice(row._id)} >{row.currency} {row.total? toCommas(row.total.toFixed(2)): row.total} </TableCell>
