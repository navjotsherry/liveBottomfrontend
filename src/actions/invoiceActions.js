...
export const getInvoice = (id) => async (dispatch)=> {
    ...
    try {
        ...
        const businessDetails = await api.fetchProfilesByUser({search: user?.result?.sub || user?.result?._id || user?.result?.googleId})
...
export const createInvoice =(invoice, history) => async (dispatch) => {
    try {
        ...
        console.log("dispatched data Invoice: ", data)
